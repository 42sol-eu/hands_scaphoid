    ...
    def test_run_in_container(self, shell_with_temp_dir):
        ...
        with patch.object(shell_with_temp_dir, 'run') as mock_run:
            ...
            mock_run.assert_called_once_with(
                expected_command, None, True, True, True
            )
            assert result == mock_result
...
class TestShellDependsOnMethod:
    ...
    def test_depends_on_running_containers(self, mock_run, mock_allow, shell_with_temp_dir):
    ...
    @patch.object(Shell, 'allow')
    @patch.object(Shell, 'run')
    def test_depends_on_missing_container(self, mock_run, mock_allow, shell_with_temp_dir):
