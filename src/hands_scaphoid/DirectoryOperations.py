...
class Directory:
    ...
    def list_files(dir_path: PathLike, extension: Optional[str] = None) -> List[str]:
        ...
        try:
            ...
            for item in path.iterdir():
                if item.is_file():
                    if extension is None or item.suffix.lstrip('.').lower() == extension.lower():
    ...
    @staticmethod
    def copy_directory(source_path: PathLike, target_path: PathLike, 
        ...
        try:
            ...
            console.print(f"[red]Permission denied copying directory:[/red] {source} → {target}")
            raise
        except Exception as e:
            console.print(f"[red]Error copying directory {source} → {target}:[/red] {e}")
            raise
    ...
    def move_directory(source_path: PathLike, target_path: PathLike) -> None:
        ...
        try:
            ...
            console.print(f"[red]Permission denied moving directory:[/red] {source} → {target}")
            raise
        except Exception as e:
    ...
    def delete_directory(dir_path: PathLike, recursive: bool = False) -> None:
        ...
        try:
            ...
            if "not empty" in str(e).lower():
                console.print(f"[red]Directory not empty (use recursive=True):[/red] {path}")
            else:
    ...
    def get_directory_size(dir_path: PathLike) -> int:
        ...
        try:
            ...
            console.print(f"[red]Permission denied accessing some files in:[/red] {path}")
            raise
        except Exception as e:
