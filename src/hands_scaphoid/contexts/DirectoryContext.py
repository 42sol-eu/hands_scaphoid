...
class DirectoryContext(Context):
    ...
    def __init__(self, path: PathLike, create: bool = True, dry_run: bool = False, enable_globals: bool = False):
        """
    ...
    def _create_path(self, resolved_path: Path) -> None:
        ...
        """
        if self.dry_run:
            console.print(f"[dim][DRY RUN] Would create directory: {resolved_path}[/dim]")
            return
    ...
    def _enter_context(self, resolved_path: Path) -> None:
        ...
        """
        if self.dry_run:
            console.print(f"[dim][DRY RUN] Would change to directory: {resolved_path}[/dim]")
            return
    ...
    def _exit_context(self, resolved_path: Path) -> None:
        ...
        """
        if self.dry_run:
            console.print(f"[dim][DRY RUN] Would restore directory: {self.previous_cwd}[/dim]")
            return
        ...
        if self.previous_cwd:
            try:
                ...
                console.print(f"[red]Error restoring directory {self.previous_cwd}:[/red] {e}")
                raise
    ...
    def list_contents(self) -> List[str]:
        ...
        """
        if self.dry_run:
            console.print(f"[dim][DRY RUN] Would list contents of: {self.resolve_path()}[/dim]")
    ...
    def list_files(self, extension: Optional[str] = None) -> List[str]:
        ...
        """
        if self.dry_run:
            console.print(f"[dim][DRY RUN] Would list files in: {self.resolve_path()}[/dim]")
    ...
    def list_directories(self) -> List[str]:
        ...
        """
        if self.dry_run:
            console.print(f"[dim][DRY RUN] Would list directories in: {self.resolve_path()}[/dim]")
    ...
    def create_subdirectory(self, name: str) -> 'DirectoryContext':
        ...
        if self.dry_run:
            console.print(f"[dim][DRY RUN] Would create subdirectory: {subdir_path}[/dim]")
    ...
    def is_empty(self) -> bool:
        ...
        """
        if self.dry_run:
            console.print(f"[dim][DRY RUN] Would check if directory is empty: {self.resolve_path()}[/dim]")
            return False
    ...
    def get_directory_size(self) -> int:
        ...
        """
        if self.dry_run:
            console.print(f"[dim][DRY RUN] Would get directory size: {self.resolve_path()}[/dim]")
            return 0
        
        return Directory.get_directory_size(self.resolve_path())
    
    def copy_to(self, target_path: PathLike, dirs_exist_ok: bool = False) -> 'DirectoryContext':
        """
        ...
        """
        if self.dry_run:
            console.print(f"[dim][DRY RUN] Would copy directory: {self.resolve_path()} → {target_path}[/dim]")
            return self
    ...
    def move_to(self, target_path: PathLike) -> 'DirectoryContext':
        ...
        """
        if self.dry_run:
            console.print(f"[dim][DRY RUN] Would move directory: {self.resolve_path()} → {target_path}[/dim]")
    ...
    def delete(self, recursive: bool = False) -> None:
        ...
        """
        if self.dry_run:
            console.print(f"[dim][DRY RUN] Would delete directory: {self.resolve_path()}[/dim]")
            return
