...
class ArchiveContext(Context):
    ...
    def __init__(
        ...
        if source and not target:
            ...
            self.source_is_archive = (
                Archive.is_archive_file(Path(source)) if source else False
            )
    ...
    def add_file(
        self, file_path: PathLike, archive_path: Optional[str] = None
    ) -> "ArchiveContext":
        """
        ...
        if self.dry_run:
            console.print(
                f"[dim][DRY RUN] Would add file to archive: {file_path}[/dim]"
            )
            return self
    ...
    def add_directory(
        self, dir_path: PathLike, archive_path: Optional[str] = None
    ) -> "ArchiveContext":
        """
        ...
        if self.dry_run:
            console.print(
                f"[dim][DRY RUN] Would add directory to archive: {dir_path}[/dim]"
            )
            return self
        ...
        if self.archive_type == "zip":
            ...
            console.print(
                f"[yellow]Note: Adding directories to TAR archives recreates the archive[/yellow]"
            )
    ...
    def list_contents(self) -> List[str]:
        ...
        if self.dry_run:
            console.print(
                f"[dim][DRY RUN] Would list archive contents: {self.resolve_path()}[/dim]"
            )
    ...
    def extract_file(
        self, file_name: str, target_path: Optional[PathLike] = None
    ) -> "ArchiveContext":
        """
        ...
        if self.dry_run:
            console.print(
                f"[dim][DRY RUN] Would extract file from archive: {file_name}[/dim]"
            )
            return self
    ...
    def extract_all(self, target_path: Optional[PathLike] = None) -> "ArchiveContext":
        ...
        if self.dry_run:
            console.print(
                f"[dim][DRY RUN] Would extract all from archive: {self.resolve_path()}[/dim]"
            )
            return self
    ...
    def get_archive_info(self) -> Dict[str, Any]:
        ...
        if self.dry_run:
            console.print(
                f"[dim][DRY RUN] Would get archive info: {self.resolve_path()}[/dim]"
            )
            return {
    ...
    def copy_to(self, target_path: PathLike) -> "ArchiveContext":
        ...
        if self.dry_run:
            console.print(
                f"[dim][DRY RUN] Would copy archive: {self.resolve_path()} → {target_path}[/dim]"
            )
            return self
        ...
        console.print(
            f"[green]Copied archive:[/green] {self.resolve_path()} → {target_path}"
        )
        return self
    ...
    def delete(self) -> None:
        ...
        if self.dry_run:
            console.print(
                f"[dim][DRY RUN] Would delete archive: {self.resolve_path()}[/dim]"
            )
            return
    ...
    def get_compression_ratio(self) -> float:
        ...
        if self.dry_run:
            console.print(
                f"[dim][DRY RUN] Would get compression ratio: {self.resolve_path()}[/dim]"
            )
            return 0.0
