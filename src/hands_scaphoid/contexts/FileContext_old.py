...
class File(Context):
    ...
    def __init__(self, path: PathLike, create: bool = True, mode: str = 'r+', encoding: str = 'utf-8', dry_run: bool = False, enable_globals: bool = False):
        """
    ...
    def _create_path(self, resolved_path: Path) -> None:
        ...
        try:
            ...
            console.print(f"[red]Permission denied creating file:[/red] {resolved_path}")
            raise
    ...
    def write_content(self, content: str, append: bool = False) -> 'File':
        ...
        if self.dry_run:
            ...
            console.print(f"[dim][DRY RUN] Would {action} file: {self.resolve_path()}[/dim]")
            return self
    ...
    def insert_line_at(self, line_number: int, content: str) -> 'File':
        ...
        """
        if self.dry_run:
            console.print(f"[dim][DRY RUN] Would insert line at {line_number}: {content[:50]}...[/dim]")
            return self
        ...
        if not self._entered:
            ...
            try:
                ...
                console.print(f"[red]Error inserting line in file {resolved_path}:[/red] {e}")
                raise
        else:
            # Context mode
