class ArchiveContextOld(Context):
        ...
        if target is None and source is not None:
            ...
            if source_path.exists() and source_path.is_file():
                ...
                if source_suffix in [
                    ...
                    ".gz",
                    ".bz2",
                ] or source_path.name.endswith((".tar.gz", ".tar.bz2")):
    ...
    def _create_path(self, resolved_path: Path) -> None:
        ...
        try:
            ...
            console.print(
                f"[red]Permission denied creating archive:[/red] {resolved_path}"
            )
            raise
    ...
    def _enter_context(self, resolved_path: Path) -> None:
        ...
        try:
            ...
            console.print(
                f"[red]Permission denied opening archive:[/red] {resolved_path}"
            )
            raise
        except Exception as e:
    ...
    def add_file(
        self, file_path: PathLike, archive_name: Optional[str] = None
    ) -> "Archive":
        """
    ...
    def add_directory(
        self, dir_path: PathLike, archive_name: Optional[str] = None
    ) -> "Archive":
        """
        ...
        if not dir_path.exists():
            console.print(
                f"[yellow]Warning: Directory does not exist:[/yellow] {dir_path}"
            )
            return self

        if not dir_path.is_dir():
            console.print(
                f"[yellow]Warning: Path is not a directory:[/yellow] {dir_path}"
            )
            return self
        ...
        try:
            ...
            console.print(
                f"[red]Error adding directory {dir_path} to archive:[/red] {e}"
            )
            raise
    ...
    def _standalone_add_file(
        ...
        if not file_path.exists() or not file_path.is_file():
            console.print(
                f"[yellow]Warning: File does not exist or is not a file:[/yellow] {file_path}"
            )
            return self
    ...
    def _standalone_add_directory(
        ...
        if not dir_path.exists() or not dir_path.is_dir():
            console.print(
                f"[yellow]Warning: Directory does not exist or is not a directory:[/yellow] {dir_path}"
            )
            return self
