...
class Directory(Context):
    ...
    def _create_path(self, resolved_path: Path) -> None:
        ...
        try:
            ...
            console.print(f"[red]Permission denied creating directory:[/red] {resolved_path}")
            raise
    ...
    def _enter_context(self, resolved_path: Path) -> None:
        ...
        try:
            ...
            console.print(f"[red]Permission denied accessing directory:[/red] {resolved_path}")
            raise
        except Exception as e:
            console.print(f"[red]Error changing to directory {resolved_path}:[/red] {e}")
            raise
    ...
    def delete(self) -> 'Directory':
        ...
        resolved_path = self.resolve_path()
        if self.dry_run:
            console.print(f"[dim][DRY RUN] Would delete directory: {resolved_path}[/dim]")
            ...
            try:
                if resolved_path.is_dir():
                    ...
                    console.print(f"[yellow]Warning: Path is not a directory:[/yellow] {resolved_path}")
            except Exception as e:
                console.print(f"[red]Error deleting directory {resolved_path}:[/red] {e}")
                raise
        return self
